...
命令结构:
  ...
  papercrawler recategorize 给已下载论文补跑分类
...
子模块:
  ...
  recategorize.py  recategorize 命令
...
app = typer.Typer(
    name="paper-dl",
    help="学术论文检索与下载工具。支持 OA 全文下载、Sci-Hub 兜底、MarkItDown 格式转换、领域感知分类。",
...
def _register_subcommands() -> None:
    ...
    from papercrawler.cli import search, download, recategorize, convert, history, config  # noqa: F401

    # search / download / batch / recategorize / convert — 直接挂到 app
    app.command("search")(search.cmd_search)
    app.command("download")(download.cmd_download)
    app.command("batch")(download.cmd_batch)
    app.command("recategorize")(recategorize.cmd_recategorize)
