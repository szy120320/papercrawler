...
def cmd_search(
    ...
    interest_threshold: float         = typer.Option(0.7,   "--interest-threshold", help="第一阶段粗筛最低分数(title-based, 默认 0.7, <此值过滤掉)"),
    semantic_min_matches: int        = typer.Option(3,    "--semantic-min-matches", help="第二阶段细筛最少命中关键词数(title+abstract+keywords 中命中 ≥ 此值才保留, 默认 3)"),
    categorize:       Optional[bool]  = typer.Option(None, "--categorize/--no-categorize", help="按 [interest.categories] 自动给论文打分类标签 (None=按配置)"),
    ...
    配置了关键词或分类,就会自动启用 --interest / --categorize / --csv。
    用 --no-interest / --no-categorize / --no-csv 可显式关闭。
    ...
    has_categories = bool(cfg.interest.categories)
    has_interest_config = has_keywords or has_categories

    # 领域打分:三态 -> 最终布尔
    if interest is None:
        interest = has_keywords
    if categorize is None:
        categorize = has_categories

    # CSV:三态 -> 路径
    if no_csv:
        csv_path = None
    elif csv_path is None and (interest or categorize):
        # 自动启用:导出到运行目录下
        csv_path = str(output_path / "_interest_filtered.csv")

    # 显示本轮配置
    if has_interest_config:
        flags = []
        if interest: flags.append("interest")
        if categorize: flags.append("categorize")
        if csv_path: flags.append(f"csv→{Path(csv_path).name}")
    ...
    async def _run_search():
        ...
        if papers and (interest or categorize):
    ...
    # 阶段2 细筛 + 自动分类(在 enrich 之后,abstract 已就绪)
    ...
    if csv_path:
        ...
        combined_path = Path("results") / "_all_filtered.csv"
